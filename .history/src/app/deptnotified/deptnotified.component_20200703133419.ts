...
export class DeptnotifiedComponent implements OnInit {
  ...
  getFormValidate( ElementChk:string ){
    ...
    if( ElementChk=="DeptProblemPoint" && this.NtfModel.DeptProblemPoint==null){
  ...
  GetUserLogin(){
    ...
    if( this.NtfModel.FormValidate=='-' &&
        this.NtfModel.FormError=='-' ){
      this.http.post(
        this.Config.urlApi + "getUser.php",
        ...
        (data:any)=>{
          ...
          if( data.length > 0 ){
            ...
            if(data[0].deptphone=="-"){ this.NtfModel.DeptPhone = null }
            else{ this.NtfModel.DeptPhone = data[0].deptphone; }
  ...
  SaveNotified(){
    if( this.NtfModel.DeptProblem!=null &&
        this.NtfModel.DeptProblemPoint!=null &&
        this.NtfModel.DeptPhone!=null ){
      this.http.post(
        this.Config.urlApi + "savedeptnotified.php",
        { dbase: this.NtfModel }
      ).subscribe(
        (data:any)=>{
          // console.log( data );
