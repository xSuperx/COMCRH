...
@Component({
  selector: 'app-reqcom',
  templateUrl: './reqcom.component.html',
  styleUrls: ['./reqcom.component.scss']
})

export class ReqcomComponent implements OnInit {
  ...
  NewAppDialog(Content){
    Swal({
      title: "ท่านต้องการลงทะเบียนขอจัดสร้าง"+
             "\nระบบฐานข้อมูลและโปรแกรมบริหารจัดการข้อมูล"+
             "\nหรือไม่ ?",
  ...
  NewCompDialog(Content){
    Swal({
      title: "ท่านต้องการลงทะเบียนแจ้งขอ"+
      ...
      async isConfirm => {
        if(isConfirm.value){
          ...
          this.RequestFrm.reset();
          this.RequestFrm.patchValue({
            ReqNbr: CruYMD,        // รหัสคำขอ
            ReqType: "C",          // ประเภทการขอ C=ขอคอมฯอุปกรณ์ต่อพ่วง
            ReqUser: null,         // ชื่อผู้ร้องขอ
            ReqUsrContact: null,   // ชื่อผู้ประสาน
            ReqUsrContactTel: null,// เบอร์ภายใน
            ReqTitle: null,        // คอมฯ หรืออุปกรณ์ที่ต้องการ
            ReqReason: null,       // เหตุผลความต้องการ
            ReqReturn: null,       // ผลทีคาดว่าจะได้รับ
            ReqResource: null,     // ทรัพยากรที่ต้องการใช้เพิ่มเติม
            ReqRequestor: "-",     // ไม่ใช่ฟิวล์นี้ในการขอคอมฯ
            ReqRisks: "-",         // ไม่ใช่ฟิวล์นี้ในการขอคอมฯ
            ReqAppConOrther: "-",  // ไม่ใช่ฟิวล์นี้ในการขอคอมฯ
            ReqForRM: "-",         // ไม่ใช่ฟิวล์นี้ในการขอคอมฯ
            ReqPartUI: "-"         // ไม่ใช่ฟิวล์นี้ในการขอคอมฯ
          });
          this.ShowContent(Content);
        }
      }
      ...
      this.uploader.onCompleteItem = (item, response, status) => {
        if(status === 200){
          ...
          if(data.State === true){
            ...
            this.http.post(
              this.appConfig.urlApi + 'SaveNewApp.php',
  ...
  SaveNewComp(){
    ...
    this.http.post(
      ...
      (data:any)=>{
        ...
        this.GetRequestComp();
        Swal({
          title: 'บันทึกคำขอคอมพิวเตอร์และอุปกรณ์ต่อพ่วง \n เสร็จสมบูรณ์ ?',
          text: '', type: 'success', allowOutsideClick: false,
          showConfirmButton: true, buttonsStyling: true,
          confirmButtonColor: '#0CC27E',
          confirmButtonText: '<i class="icon icon-star"></i> ตกลง',
